...
def get_alazar_parameters(daq_params=None):
    ...
    #
    alazar_params.post_trigger_samples =8192 #4192 #8192  # # 2048 
    ...
    alazar_params.records_per_buffer = min(1*3072, alazar_params.num_total_records)# #3072 #8192 For 8192 I changed 2*3072 to 1*3072
...
#    start_from(0) #serra commented out
...
#    reset_proteus_clock(slotId=5)
    ...
    try:
        ...
        #print("\n2")
        srs_dg535.set_trigger_level('high')#dg535_control.set_state(1) ## shifting this line from L227 solved sequence timing and averaging window shift issues.
        ...
        while (buffersCompleted < buffers_per_acquisition and not ats.enter_pressed()):
            ...
            rec_all_het = np.einsum('ijk, lik->lj', rec_all[:,:, index_avg_start:index_avg_end], v)
...
#        generator.proteus_close()
        ...
        srs_dg535.set_trigger_level('low')#dg535_control.set_state(0) #serra added and commented out
...
#### Stolen from DVK's code
def acquire_data_het_2q(daq_params, alazar_params, board, ssm_if_1=-0.04, ssm_if_2=0.10692 , deg_1=0, deg_2=0 ,verbose=True):
    rec_avg_all = []
    rec_readout = []
...
#    reset_proteus_clock(slotId=5)
    ...
    try:
        ...
        #alexandria commented out above line and replaced it below 11/01/2023
        ...
        #print("\n3\n")
        #print(buffers_per_acquisition)
        #with tqdm(total=buffers_per_acquisition, desc="Averaging", bar_format="{l_bar}{bar} [time left: {remaining}]") as pbar:
        ...
        while (buffersCompleted < buffers_per_acquisition and not ats.enter_pressed()):
            ...
            omega_1 = np.arange(index_avg_start, index_avg_end) * (2 * np.pi * ssm_if_1)
            v_cos_1 = np.cos(omega_1+((np.pi*deg_1)/180)) #/ np.sqrt(alazar_params.post_trigger_samples)
            v_sin_1 = np.sin(omega_1+((np.pi*deg_1)/180)) #/ np.sqrt(alazar_params.post_trigger_samples)
            v_1 = np.array([[v_cos_1, -v_sin_1], [v_sin_1, v_cos_1]])
            rec_all_het_1 = np.einsum('ijk, lik->lj', rec_all[:,:, index_avg_start:index_avg_end], v_1)
             
            omega_2 = np.arange(index_avg_start, index_avg_end) * ((2 * np.pi * ssm_if_2))
            v_cos_2 = np.cos(omega_2+((np.pi*deg_2)/180)) #/ np.sqrt(alazar_params.post_trigger_samples)
            v_sin_2 = np.sin(omega_2+((np.pi*deg_2)/180)) #/ np.sqrt(alazar_params.post_trigger_samples)
            v_2 = np.array([[v_cos_2, -v_sin_2], [v_sin_2, v_cos_2]])
            rec_all_het_2 = np.einsum('ijk, lik->lj', rec_all[:,:, index_avg_start:index_avg_end], v_2)
            ...
            #rec_avg_all[idx] = np.mean(rec_all[idx], axis=0) # is this just the avg of the last loop?
            #rec_readout[idx] = np.concatenate((rec_readout[idx], np.mean(rec_all[idx][:,index_avg_start:index_avg_end], axis=1)))
                rec_readout_1[idx] = np.concatenate((rec_readout_1[idx], rec_all_het_1[idx]))
                rec_readout_2[idx] = np.concatenate((rec_readout_2[idx], rec_all_het_2[idx]))
...
#        generator.proteus_close()
        # stop SRS DG535 triggers
        srs_dg535.set_trigger_level('low') #dg535_control.set_state(0) #alexandria recently commented out 11/01/2023
    #print("\ndaq_alazar Troubleshoot stuck at this step 16")
    # Compute the total transfer time, and display performance information.
    #print("H")
    if verbose:
        ...
        print("Transferred %d bytes (%f bytes per sec)" %
              (bytesTransferred, bytesPerSec))
    #print("I")
    return (rec_avg_all, rec_readout_1, rec_readout_2, rec_all, rec_all_het_1, rec_all_het_2)
...
def acquire_data(daq_params, alazar_params, board):
    ...
    try:
        ...
        while (buffersCompleted < buffers_per_acquisition and not
            ...
            # buffers to be filled by the board.
            buffer = buffers[buffersCompleted % len(buffers)]
            board.waitAsyncBufferComplete(buffer.addr, timeout_ms=500000)# was 5000 earlier
            buffersCompleted += 1
            bytesTransferred += buffer.size_bytes
    ...
    return (rec_avg_all, rec_readout, rec_all) #added rec_all 12/8/2022

def acquire_data_raw(daq_params, alazar_params, board):
    rec_avg_all = []
    rec_readout = []
    ...
    rec_all_raw = [None]*2
    rec_all_raw_ave = np.zeros((2,0,post_trigger_samples))
    rec_all_raw_ave = np.zeros((buffers_per_acquisition,2,records_per_buffer,post_trigger_samples))
    rec_avg_all = [None]*2
    rec_readout = [[]]*2
...
#            rec_all = rotate_iq(daq_params.iq_angle_deg, rec_all_raw)
            rec_all = rec_all_raw
            #
            rec_all_raw_ave[buffersCompleted-1,:,:,:] = rec_all_raw
...
#                rec_readout[idx] = np.concatenate((rec_readout[idx], np.mean(rec_all[idx][:,index_avg_start:index_avg_end], axis=1)))
    ...
    return (rec_avg_all, rec_readout,rec_all_raw_ave)
# END acquire_data_raw
