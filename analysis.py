...
#    p_e = p_readout[1][0:int(length/2)]
    p_e = p_readout[2][int(length/2):int(length)] #use f state to readout e state pop
...
def p_readout_scaled(p_readout):
    ...
    for i in range(3):
        fidelity_matrix[i,:] = p_readout_three_pnt_msmt[i,:]/np.sum(p_readout_three_pnt_msmt[i,:])
...
def fit_double_exp_decay(x_vals, y_vals, guess_vals=None):
    if guess_vals is None:
        ...
        guess_gamma2 = 6.0
        guess_offset = 0
        guess_vals = [guess_amplitude1, guess_gamma1, guess_amplitude2, guess_gamma2, guess_offset]
...
def fit_three_gaussian(hist_bins, hist_counts):
    ...
    maxInd = argrelextrema(hist_counts,np.greater, order = 10) # serach a local maxima within [N-order, N+order] range
...
#    plt.ylim(0,2000)
    ...
    threshold_01_index = mu0_fit_index + np.argmin(abs(fitfun.gaussian(hist_bins[mu0_fit_index:mu1_fit_index], *fit[0][0::3])-fitfun.gaussian(hist_bins[mu0_fit_index:mu1_fit_index], *fit[0][1::3]))) 
    threshold_12_index = mu1_fit_index + np.argmin(abs(fitfun.gaussian(hist_bins[mu1_fit_index:mu2_fit_index], *fit[0][1::3])-fitfun.gaussian(hist_bins[mu1_fit_index:mu2_fit_index], *fit[0][2::3])))
...
def fit_two_gaussian(hist_bins, hist_counts):
    ...
    if len(maxInd[0]) == 2:
        ...
        guess_vals = [2000,5000,155,162,2.5,1.5] ##Not sure about this guess value, which ones to exclude
...
#            break        
    ...
    threshold_01_index = mu0_fit_index + np.argmin(abs(fitfun.gaussian(hist_bins[mu0_fit_index:mu1_fit_index], *fit[0][0::2])-fitfun.gaussian(hist_bins[mu0_fit_index:mu1_fit_index], *fit[0][1::2]))) 
...
def fit_sine(x_vals, y_vals, guess_vals=None):
    ...
    fit = curve_fit(fitfun.sine, x_vals, y_vals, p0=guess_vals)
    y_vals_fit = fitfun.sine(x_vals, *fit[0])
