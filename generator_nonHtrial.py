...
class Pulse:
    ...
    def __init__(self,duration, start, amplitude, ssm_freq=None, phase=0, gaussian_bool=False, 
                 phase_ini=0, t_loop=1E-20, ff=None,detunlinear=0.0,detun_NH=0,path_sign=1,detun_NH_phase=0,jmin=0, derived_gaussian_bool = False):
    ...
    def copy(self):
        ...
        if self.ssm_bool:
            return Pulse( self.duration, self.start, self.amplitude, self.ssm_freq, self.phase, self.gaussian_bool, 
                         self.phase_ini, self.t_loop, self.ff,self.detunlinear,self.detun_NH,self.path_sign,self.detun_NH_phase,self.jmin,self.derived_gaussian_bool)
        else:
...
#    detun=pulse.detun
    ...
    bmin=1+pulse.jmin # for NHH qubit parameter driving purpose. jmin doesn't represent the true value of the minium J value; instead, it's the ratio of the minimum value
    # to the maximum J value. That is to say, if we pick jmin a small value (e.g., 0.01), we obtain EP encircling; if we pick jmin = 1, we only obtain a freqeuncy-varying drive (with constant amplitude)
    ...
    if ff==None:
          ...
          if pulse.derived_gaussian_bool:
              ...
              addition *= derived_gaussian #Should I add the coefficient here?
