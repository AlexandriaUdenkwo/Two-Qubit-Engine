...
ge_amp_setting = 1 #1.9 for 1in driving
...
#    file_length = 10000 * (np.int(totlength/10000)+1)
#    
...
#        ringupdown_seq.add_sweep(channel=4, sweep_name='start', start=0, stop=-sweeptime, initial_pulse=pi_ge)
    ...
    ringupdown_seq.add_sweep(channel=1,sweep_name='none',initial_pulse=main_pulse)# , marker=2
#    
...
def T1_ge_tunable_dissipation(num_steps= 51, sweeptime = 50000,ssm_ge=-.15,pi_ge_time=100,ssm_coax=-0.25, amp_coax=0.25, ssm_Q2=0, amp_Q2=0, q=1, ifload = 1): #this is pulsed readout to ring up and ring down cavity dfor e state
    totlength = sweeptime + 4000
    file_length = 10000 * (np.int(np.ceil(totlength/10000))+1)
...
#        # off-resonance drive for filter (of Q1)
...
#        
        # off-resonance drive for Q2
        q2_off_resonance_drive = Pulse(start=file_length-readout_dur, duration=0, amplitude=amp_Q2, ssm_freq=ssm_Q2, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=3, sweep_name='width', start=-500, stop=-coaxtime,initial_pulse=q2_off_resonance_drive)
        q2_off_resonance_drive.phase = 90+phase_offset
        the_seq.add_sweep(channel=4, sweep_name='width', start=-500, stop=-coaxtime,initial_pulse=q2_off_resonance_drive)
...
#    
    ...
    if True:
        ...
        plt.plot(channel3_ch[10,file_length-readout_dur-coaxtime+12500:file_length-readout_dur])
...
def T1_ge_tunable_dissipation_gaussian_pulse(num_steps= 51, sweeptime = 50000,ssm_ge=-.15,pi_ge_time=100,ssm_coax=-0.25, amp_coax=1, ssm_Q2=0.1, amp_Q2=0.2, ROIF=0.1, q=0, ifsideband=1, ifload = 1): #this is pulsed readout to ring up and ring down cavity dfor e state
...
#        plt.plot(channel1_ch[1,file_length-readout_dur-coaxtime-3000:file_length-readout_dur-coaxtime+4500+4000])
        plt.plot(channel4_ch[2,63000:-5000])
...
def pi_nopi(coef=0,off=0,pi_ge=24,ssm_ge = -0.04575, q=0, ROIF=0.01): #this is pulsed readout to ring up and ring down cavity dfor e state
...
#    ssm_ge = ssm_ge_setting
    ...
    readout_amp = 0.035#ro_amp#0.5# 1
    readout_dur = 5000# ro_pulse_dur#ro_dur #8000#13000 #1000
#    phase_offset = mixer_offset
#    phase_offset_ef = mixer_offset_ef
 
    
    pi_2x = Pulse(start=file_length-readout_dur, duration=-pi_ge*coef, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
    ringupdown_seq.add_sweep(channel=4, sweep_name='none', start=0, stop=-sweep_time,initial_pulse=pi_2x)
...
#    ringupdown_seq.add_sweep(channel=1, marker=2, sweep_name='none', initial_pulse=gate_trigger )
    ...
    #Q1 Readout
    if q == 1:
        main_pulse = Pulse(start = file_length- readout_dur,duration= readout_dur, amplitude= readout_amp,ssm_freq=ROIF, phase=-file_length*ROIF*360 ) 
        ringupdown_seq.add_sweep(channel=1, sweep_name='none',initial_pulse=main_pulse) 
...
#    ssm_ge = ssm_ge_setting
    ...
    readout_amp = 0.035 #0.261 #1#0.5# 1
...
def spectroscopy_ge(num_steps=101,ssm_start=-.05,ssm_stop=-.15,spec_amp=.5,ROIF1 =0,ROIF2 =0,q=0): #this is pulsed readout to ring up and ring down cavity dfor e state
...
#    
   ...
   # ringupdown_seq.add_sweep(channel=1, sweep_name='none',initial_pulse=main_pulse)
    ...
    main_pulse = Pulse(start = file_length- readout_dur,duration= readout_dur, amplitude= readout_amp,ssm_freq=ROIF1, phase=0 ) 
    ringupdown_seq.add_sweep(channel=1, sweep_name='none',initial_pulse=main_pulse)
    #main_pulse.phase = 90
    #ringupdown_seq.add_sweep(channel=2, sweep_name='none',initial_pulse=main_pulse)
    #Q2 Readout
...
def spectroscopy_ge_with_cavityDrive(num_steps=10,ssm_start=-.1,ssm_stop=-.2,spec_amp=.05,amp_cavity=0.1, ssm_cavity=0.1, ROIF=0.1, q = 1): #this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
#    num_steps = 51
    the_seq = Sequence(file_length, num_steps) #this creates something called rabi_seq that is an instance of a sequence class
...
#    ssm_ge = ssm_ge_setting
    ...
    if q == 1: #qubit 1
        rabi_ge = Pulse(start=file_length-readout_dur-10, duration=-7000, amplitude=spec_amp, ssm_freq=0, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=3, sweep_name='ssm_freq', start=ssm_start, stop=ssm_stop,initial_pulse=rabi_ge)
#        rabi_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=4, sweep_name='ssm_freq', start=ssm_start, stop=ssm_stop,initial_pulse=rabi_ge)

    # on-resonance RO cavity drive
#        cavity_drive = Pulse(start=file_length-readout_dur, duration=-7000, amplitude=amp_cavity) #pulse is also a class p is an instance
#        the_seq.add_sweep(channel=1, sweep_name='none',initial_pulse=cavity_drive)
    # off-resonance RO cavity drive
        cavity_drive = Pulse(start=file_length-readout_dur, duration=-7000, amplitude=amp_cavity, ssm_freq=ssm_cavity, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=1, sweep_name='none',initial_pulse=cavity_drive)
#        cavity_drive.phase = 90
#        the_seq.add_sweep(channel=2, sweep_name='none',initial_pulse=cavity_drive)
...
#    
    if q == 0: #qubit 2
        ...
        the_seq.add_sweep(channel=4, sweep_name='ssm_freq', start=ssm_start, stop=ssm_stop,initial_pulse=rabi_ge)
#        rabi_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=2, sweep_name='ssm_freq', start=ssm_start, stop=ssm_stop,initial_pulse=rabi_ge)
...
#        the_seq.add_sweep(channel=2, sweep_name='none',initial_pulse=cavity_drive)
    ...
    for k in np.arange(num_steps):
        ...
        # for off-resonance drives to qubit and RO cavity, both of which have the same duration and starting time
#                  pulseLength = np.int_(2*width + sweeptime * k/(num_steps-1))
            pulseLength = np.int_(sweeptime) # * k/(num_steps-1))
            pulseStart = file_length - readout_dur - pulseLength #file_length-pi_ge_time-pulseLength - 1000
            tdata = np.arange(pulseLength)
            pdata2 = np.zeros(pulseLength)

            # gaussian shape                    
            pdata2[0:width] = amp_cavity * np.exp(-(tdata[0:width]-width)**2/(2*sigma**2))
...
#                    secondStop = np.int_(width+sweeptime * k/(num_steps-1))
            ...
            pdata2[width:secondStop] = amp_cavity
            
            pdata2[secondStop:pulseLength] = (amp_cavity *
                  np.exp(-(tdata[secondStop:pulseLength]-secondStop)**2/(2*sigma**2)))
                  
            
            the_seq.channel_list[0][0][k,pulseStart:pulseStart+pulseLength] *= pdata2
#            the_seq.channel_list[1][0][k,pulseStart:pulseStart+pulseLength] *= pdata2
...
#    #trigger pulse to open switch gate
#    gate_trigger = Pulse(start=file_length- readout_dur, duration=readout_dur, amplitude=1)
#    the_seq.add_sweep(channel=1, marker=2, sweep_name='none', initial_pulse=gate_trigger )
...
#        plt.plot(channel1_ch[5,file_length-3500:file_length-2500])
#        plt.plot(channel3_ch[5,file_length-10500:file_length-9500])
#        plt.show()
...
def spectroscopy_ROcavity(num_steps=3,ssm_ge=-.15,pi_ge_time=20,spec_amp=.05,ROIF=-0.01, estate = 1, q = 1): #this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 6000
#    num_steps = 51
    the_seq = Sequence(file_length, num_steps) #this creates something called rabi_seq that is an instance of a sequence class
...
def raman_ROcavity(num_steps=3,ssm_ge=-.15,ge_amp = 1, amp_cavity=0.1, ssm_cavity=-0.015, rabi_time=20000, ROIF=0.1, ifsideband=1, q = 0): #this is pulsed readout to ring up and ring down cavity dfor e state
...
#    ssm_ge = ssm_ge_setting
    ...
    readout_amp = 0.035 #0.261 #spec_amp #1#0.5# 1  
    readout_dur = ro_pulse_dur#2000 #13000 #1000
    phase_offset = mixer_offset
...
#    ge_amp = 0.1
    ...
    if q == 1: #qubit 1
        rabi_ge = Pulse(start=file_length-readout_dur, duration=-rabi_time, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=3, sweep_name='none', initial_pulse=rabi_ge)
#        rabi_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=4, sweep_name='none', initial_pulse=rabi_ge)

        # off-resonance RO cavity drive
        cavity_drive = Pulse(start=file_length-readout_dur, duration=-rabi_time, amplitude=amp_cavity, ssm_freq=ssm_cavity, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=1, sweep_name='none',initial_pulse=cavity_drive)
#        cavity_drive.phase = 90
#        the_seq.add_sweep(channel=2, sweep_name='none',initial_pulse=cavity_drive)
...
#        the_seq.add_sweep(channel=2, sweep_name='none',initial_pulse=cavity_drive)
    ...
    if ifsideband == 1:
        for k in np.arange(num_steps):
            ...
            # for off-resonance drives to qubit and RO cavity, both of which have the same duration and starting time
#                  pulseLength = np.int_(2*width + sweeptime * k/(num_steps-1))
                pulseLength = np.int_(sweeptime)# for qubit drive
                pulseLength2 = np.int_(sweeptime + pulse_len_add) # for RO cavity drive
                pulseStart = file_length - readout_dur - pulseLength #file_length-pi_ge_time-pulseLength - 1000
                pulseStart2 = file_length - readout_dur - pulseLength2 #file_length-pi_ge_time-pulseLength - 1000
...
#                    secondStop = np.int_(width+sweeptime * k/(num_steps-1))
                secondStop = np.int_(-width+sweeptime)
                secondStop2 = np.int_(-width+(sweeptime+pulse_len_add))
...
#                the_seq.channel_list[2][0][k,pulseStart:pulseStart+pulseLength] *= pdata1 # for Q1
                the_seq.channel_list[3][0][k,pulseStart:pulseStart+pulseLength] *= pdata1 # for Q2
...
#    plt.plot(channel3_ch[0,7400:7500])
#    plt.show()
#        plt.plot(marker1[0])
#        plt.show()
#        channel = channel1_ch + channel3_ch + marker1
...
def ramsey_raman(num_steps = 101,t1_time = 100000,pi_echo_coef=0,osc=0,ssm_ge=-0.15,pi_ge=20,ssm_coax=-0.25, amp_coax=1, ssm_Q2=0.1, amp_Q2=0.2, ROIF=0.1, q=0, ifsideband=1, ifload = 1): #this is pulsed readout to ring up and ring down cavity dfor e state
...
#    ssm_ge = 0.3885
    ...
    coaxtime = t1_time#-5000 #500*4
    pulse_len_add = 0# 2000
...
def ramsey_coax_drive(num_steps = 51,t1_time = 1000,pi_echo_coef=0,osc=0,ssm_ge=-0.15,ssm_coax=-0.27,amp_coax=1,pi_ge=20,q=0): #this is pulsed readout to ring up and ring down cavity dfor e state
#    file_length = 10000*(np.int(t1_time/10000)+1) #64000
    totlength = t1_time + 4000
    file_length = 10000 * (np.int(totlength/10000)+2)
    #file_length = 100000#100000# 
...
#        the_seq.add_sweep(channel=2, sweep_name='width', start=-500, stop=-coaxtime,initial_pulse=coax_drive)
        ...
        ##pi/2 pulse
        t2_ge = Pulse(start=file_length-readout_dur-pi_ge/2-pi_echo_coef*pi_ge, duration=-pi_ge/2, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=4, sweep_name='start', start=0, stop=-t1_time,initial_pulse=t2_ge)
#        t2_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=2,  sweep_name='start', start=0, stop=-t1_time,initial_pulse=t2_ge)
        
        #pi pulse echo
        t2_ge = Pulse(start=file_length-readout_dur-pi_ge/2, duration=-pi_ge*pi_echo_coef, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=4, sweep_name='start', start=0, stop=-t1_time/2,initial_pulse=t2_ge)
#        t2_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=2,  sweep_name='start', start=0, stop=-t1_time/2,initial_pulse=t2_ge)
        ...
        if osc !=0:
            ...
            the_seq.add_sweep(channel=4, sweep_name='phase', start=0, stop=osc*360,initial_pulse=t2_ge)
            t2_ge.phase = 90+phase_offset
#            the_seq.add_sweep(channel=2,  sweep_name='phase', start=0, stop=osc*360,initial_pulse=t2_ge)
        elif osc == 0:
            t2_ge = Pulse(start=file_length-readout_dur, duration=-pi_ge/2, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
            the_seq.add_sweep(channel=4, sweep_name='none',initial_pulse=t2_ge)
#            t2_ge.phase = 90+phase_offset
...
#        the_seq.add_sweep(channel=4, sweep_name='width', start=-500, stop=-coaxtime,initial_pulse=coax_drive)
        ...
        ##pi/2 pulse
        t2_ge = Pulse(start=file_length-readout_dur-pi_ge/2-pi_echo_coef*pi_ge, duration=-pi_ge/2, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=3, sweep_name='start', start=0, stop=-t1_time,initial_pulse=t2_ge)
#        t2_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=4,  sweep_name='start', start=0, stop=-t1_time,initial_pulse=t2_ge)
        
        #pi pulse echo
        t2_ge = Pulse(start=file_length-readout_dur-pi_ge/2, duration=-pi_ge*pi_echo_coef, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
        the_seq.add_sweep(channel=3, sweep_name='start', start=0, stop=-t1_time/2,initial_pulse=t2_ge)
#        t2_ge.phase = 90+phase_offset
#        the_seq.add_sweep(channel=4,  sweep_name='start', start=0, stop=-t1_time/2,initial_pulse=t2_ge)
        ...
        if osc !=0:
            t2_ge = Pulse(start=file_length-readout_dur, duration=-pi_ge/2, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
            the_seq.add_sweep(channel=3, sweep_name='phase', start=0, stop=osc*360,initial_pulse=t2_ge)
#            t2_ge.phase = 90+phase_offset
#            the_seq.add_sweep(channel=4,  sweep_name='phase', start=0, stop=osc*360,initial_pulse=t2_ge)
        elif osc == 0:
            t2_ge = Pulse(start=file_length-readout_dur, duration=-pi_ge/2, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
            the_seq.add_sweep(channel=3, sweep_name='none',initial_pulse=t2_ge)
#            t2_ge.phase = 90+phase_offset
#            the_seq.add_sweep(channel=4,  sweep_name='none',initial_pulse=t2_ge)
...
#    the_seq.add_sweep(channel=1, marker=2, sweep_name='none', initial_pulse=gate_trigger )
    ...
    #Readout
    main_pulse = Pulse(start = file_length- readout_dur,duration= readout_dur, amplitude= readout_amp, ssm_freq=ROIF1, phase=0) #original readout_amp=1, duration = 1000 #-500     -1000
...
#    the_seq.channel_list[0][1] = qubit_gate
    ...
    if True:
        ...
        channel4_ch = the_seq.channel_list[3][0]
        marker1 = the_seq.channel_list[0][2]
        plt.plot(channel1_ch[50,file_length-readout_dur-coaxtime:file_length-readout_dur])
#        plt.imshow(channel2_ch[0:200,6800:7000], aspect='auto', extent=[6800,7000,200,0])
#        plt.show()
#        channel = channel1_ch + channel3_ch + marker1
#        plt.figure()
...
#    ssm_ge = ssm_ge_setting
    ...
    rotDurationEF = np.int(rotangleEF/np.pi * pi_ef_time)
    width = np.int(sigma*5/2)
    ...
    if ifsideband == 1:
        for k in np.arange(num_steps):
            pulseLength = np.int(2*width + sweeptime * k/(num_steps-1))
            ...
            secondStop = np.int(width+sweeptime * k/(num_steps-1))
...
#    ssm_ge = ssm_ge_setting
    ...
    if ifsideband == 1:
        for k in np.arange(num_steps):
            pulseLength = np.int(2*width + sweeptime)
            ...
            secondStop = np.int(width+sweeptime)
            pdata[width:secondStop] = amp
            pdata[secondStop:pulseLength] = (amp *
                  np.exp(-(tdata[secondStop:pulseLength]-secondStop)**2/(2*sigma**2)))
...
#    ssm_ge = ssm_ge_setting
    ...
    pi2_ge = Pulse(start=file_length-3050-pi_ge_time-3000, duration=-pi2_ge_time*np.int(GErotangle*2/np.pi), amplitude=ge_amp, ssm_freq=ssm_ge, phase=0) #pulse is also a class p is an instance
...
def angleSweep_f0toe1_dephasing(ave=5, singleave=5, transtime = 0, sigma=0, angle = np.linspace(0,np.pi,10),
    ...
    for l in np.arange(ave):
        for k in np.arange(np.size(angle)):
            actualAngle[k] = np.pi/pi_ef_time*np.int(angle[k]/np.pi * pi_ef_time)
...
def PowerSweep_ge_dephasingAmp_withave_thermalReadout(ave=5, att = np.arange(40,0-0.1,-2),
    ...
    for m in np.arange(ave):
        for k in np.arange(np.size(att)):
            ...
            vc.set_attenuation(np.int(att[k]))
...
#    alpha = np.array([3]) 
    ...
    ch_a_g_reshape = np.reshape(ch_a_g,[np.int(np.size(ch_a_g)/(num_steps)),num_steps])
    ch_b_g_reshape = np.reshape(ch_b_g,[np.int(np.size(ch_b_g)/(num_steps)),num_steps])
    ch_a_g_correct = np.zeros([np.int(np.size(ch_a_g)/(num_steps)),num_steps])
    ch_b_g_correct = np.zeros([np.int(np.size(ch_b_g)/(num_steps)),num_steps])
    
    ch_a_e_reshape = np.reshape(ch_a_e,[np.int(np.size(ch_a_e)/(num_steps)),num_steps])
    ch_b_e_reshape = np.reshape(ch_b_e,[np.int(np.size(ch_b_e)/(num_steps)),num_steps])
    ch_a_e_correct = np.zeros([np.int(np.size(ch_a_e)/(num_steps)),num_steps])
    ch_b_e_correct = np.zeros([np.int(np.size(ch_b_e)/(num_steps)),num_steps])
...
def timeAndPower_sweep_acStark_thermal(singleAve=10, acStark_amp = 0.05, width = 200,
    ...
    for k in np.arange(np.size(start)):
        ...
        for m in np.arange(np.size(att)):
            ...
            vc.set_attenuation(np.int(att[m]))
...
#    full_tomo()
#    ramsey_coax_drive()
#    T1_ge_tunable_dissipation()
#    ch12_leakage()
#    T1_ge_tunable_dissipation_gaussian_pulse()
#    spectroscopy_ge_with_cavityDrive()
#    spectroscopy_ROcavity()
#    raman_ROcavity()
#    rabi_ge_rotary()
#    cpmg()